...
SERVICE_NAME="registrazione_utente"

#GAUGE: RESPONSE TIME OF REGISTRAZIONE_UTENTE
REGISTRAZIONE_UTENTE_RESPONSE_TIME=prometheus_client.Gauge(
    'registrazione_utente_response_time',
    'Response time for registrazione_utente',
    ['service','hostname']
)

#COUNTER: Number of invocations of Registrazione_utente
REGISTRAZIONE_UTENTE_INVOCATIONS=prometheus_client.Counter(
    'registrazione_utente_invocations_total',
    'total number of invocations for Registrazione_utente',
    ['service','hostname']
)
...
class UserService(protoBuffer_pb2_grpc.UserServiceServicer):
    ...
    def Registrazione_utente(self,request,context):
        ...
        REGISTRAZIONE_UTENTE_INVOCATIONS.labels(service=SERVICE_NAME,hostname=HOSTNAME).inc()
        ...
        try:
            ...
            REGISTRAZIONE_UTENTE_RESPONSE_TIME.labels(service=SERVICE_NAME, hostname=HOSTNAME).set(end_time - start_time)           
            return response    
        except Error as e:
...
def start_exporter():
    ...
    #Porta su cui esporre le metriche
    print(f"Prometheus metrics are available at http://{HOSTNAME}:8000/metrics")
    while True:
        time.sleep(1) #mantiene il thread vivo
...
def serve():
    ...
    #AVVIO IL THREAD PER L'EXPORTER PROMETHEUS
