...
SERVICE_NAME="collezione_dati"

# Metriche Prometheus
REQUEST_RESPONSE_TIME = prometheus_client.Gauge(
    'data_collector_response_time',
    'Response time for data collection requests',
    ['service', 'hostname']
)
...
def start_exporter():
    ...
    print(f"Prometheus metrics are available at http://{HOSTNAME}:8001/metrics")
    while True:
    ...
    while True:
        ...
        for email,ticker in utenti:
            ...
            try:
                ...
                # Incremento del contatore solo se tutte le operazioni hanno avuto successo
                TOTAL_REQUESTS.labels(service=SERVICE_NAME, hostname=HOSTNAME).inc()
                ...
                produce_sync(producer, topic1, json.dumps(message))
                 ...
                 # Calcola e aggiorna il tempo di risposta solo in caso di successo
                end_time = time.time()
                response_time = end_time - start_time
                REQUEST_RESPONSE_TIME.labels(service=SERVICE_NAME, hostname=HOSTNAME).set(response_time)
            except Error as e:
